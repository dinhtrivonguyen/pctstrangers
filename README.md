...
const broadcast = require('./extension/broadcast');
			...
			tools.findInWaitRoom(mongo, sender, function(waitstate) {
				tools.findPartnerChatRoom(mongo, sender, function(sender2, haveToReview, role, data) {
					...
					//	sendButtonMsg(sender, la.FIRST_COME, true, true);
						sendTemplateMsg(sender, "Chào mừng đến với PCT Strangers", "Lễ thất tịch này bạn đã sẵn sàng tìm người ấy chưa? Nhập ký tự bất kỳ để bắt đầu!");
						return;
					}
					...
					if (!waitstate && sender2 == null) {
						if (command === la.KEYWORD_BATDAU) {
							...
							sendTextMessage(sender, la.HELP_TXT);
						...
						} else if (command === la.KEYWORD_TIEUCHI){
							sendButtonMsg(sender, "Bạn muốn trò chuyện với con trai hay con gái?", false, false, false, true);
						} else if (command === la.KEYWORD_BROADCAST){
							sendBroadcastMessage(sender, la.BROADCAST_TXT);
						} else if (!event.read) {
							sendButtonMsg(sender, la.HUONG_DAN, true, false);
						}
					}
					...
					//Đã vào WR và đang đợi
						...
						//	sendButtonMsg(sender, la.END_CHAT, true, true);
							sendTemplateMsg(sender, "Bạn đã kết thúc trò chuyện với người ấy ", la.END_CHAT);
						} else if (command === la.KEYWORD_HELP) {
							sendTextMessage(sender, la.HELP_TXT);
						} else if (command === la.KEYWORD_CAT) {
							gifts.sendCatPic(sender, null, true);
						...
						//	sendButtonMsg(sender, la.WAITING, false, true);
							sendTemplateMsg(sender, "Đợi chút nha!", la.WAITING);
						}
					}
					...
					//Đang vào chat
						...
						//	sendTextMessage(sender, la.BATDAU_ERR_ALREADY);
							sendTemplateMsg(sender, "Bạn đang ở trong cuộc trò chuyện với người ấy ", la.BATDAU_ERR_ALREADY);
						} else if (command === la.KEYWORD_HELP) {
							sendTextMessage(sender, la.HELP_TXT);
						} else if (command === la.KEYWORD_CAT) {
...
var connect2People = function(id, target, wantedGender, isFleur = false) {
	...
	sendTemplateMsg(id, (isFleur ? la.START_CHAT_FLEUR : la.START_CHAT));
	sendTemplateMsg(target, (isFleur ? la.START_CHAT_FLEUR : la.START_CHAT));
}
...
var sendButtonMsg = function(sender, txt, showStartBtn, showHelpBtn, showRpBtn = false, showQuickBtn = false) {
	var btns = [];
...
var sendTemplateMsg = function(sender, title, subtitle) {
	sendFacebookApi(sender, sender, {
		"attachment": {
			"type": "template",
			"payload": {
...
var sendBroadcastMessage = function(sender, txt) {
	try {
			broadcast.sendBroadcast(co.FB_PAGE_ACCESS_TOKEN, txt, function(success) {
...
var sendBroadcastTemplate = function(sender, title, subtitle, image_url, web_url) {
broadcast.send(co.FB_PAGE_ACCESS_TOKEN,	{
      "attachment":{
        "type":"template",
        "payload":{
          "template_type":"generic",
